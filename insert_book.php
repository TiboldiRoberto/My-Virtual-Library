<?php 
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_FILES['cover_image']) && isset($_FILES['pdf'])) {
        ...
        if ($error == 0 && $error1 == 0) {
			if ($img_size > 1250000) {
				...
				if (in_array($img_ex_lc, $allowed_exs)) {
                    ...
                    if(!empty($title) && !empty($author) && !is_numeric($subject))
                    {
                        ...
                        $query = "INSERT INTO books (title,author,language,subject,year,nr_pages,insert_date,cover_image,pdf) VALUES ('$title','$author','$language','$subject','$year','$nr_pages','$date','$img_new_name','$pdf_new_name')";
			...
			$em = "'Exceeds php.ini upload_max_filesize of '.$upload_max_size.'MB'";
			header("Location: index.php?error=$em");
		}
